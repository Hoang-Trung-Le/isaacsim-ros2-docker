...
# Start headless simulation application
simulation_app = SimulationApp(CONFIG)

# ROS2 Bridge and Isaac Sim imports - after SimulationApp is initialized
import omni
import numpy as np
...
from isaacsim.core.utils.prims import set_targets, is_prim_path_valid
...
camera = Camera(
    ...
    position=np.array([2.0, 0.0, 1.5]),  # Front camera position
    frequency=20,
    resolution=(640, 480),
...
camera_side = Camera(
    ...
    position=np.array([0.0, 2.0, 1.5]),  # Side camera position
    frequency=20,
    resolution=(640, 480),
    orientation=rot_utils.euler_angles_to_quats(np.array([0, 0, -90]), degrees=True),
)
camera_side.initialize()

# Create top camera
camera_top = Camera(
    prim_path="/World/Camera_Top",
    position=np.array([0.0, 0.0, 3.0]),  # Top camera position
    frequency=20,
    resolution=(640, 480),
...
# Create a reference cube for visualization
from pxr import UsdGeom, Gf

stage = simulation_context.stage
UsdGeom.Cube.Define(stage, "/World/Cube")
UsdGeom.Xformable(stage.GetPrimAtPath("/World/Cube")).AddTranslateOp().Set(
    Gf.Vec3d(0, 0, 0.5)
)

############### Calling Camera publishing functions ###############

# Set up the approximate frequency for the publishers
approx_freq = 30

# Front camera publishers
...
# Side camera publishers
publish_camera_tf(camera_side)
publish_camera_info(camera_side, approx_freq)
publish_rgb(camera_side, approx_freq)
publish_depth(camera_side, approx_freq)
publish_pointcloud_from_depth(camera_side, approx_freq)

# Top camera publishers
publish_camera_tf(camera_top)
publish_camera_info(camera_top, approx_freq)
publish_rgb(camera_top, approx_freq)
publish_depth(camera_top, approx_freq)
publish_pointcloud_from_depth(camera_top, approx_freq)

####################################################################

print("\nTo view the camera streams on a remote machine:")
print("$ ros2 run rqt_image_view rqt_image_view")
print("\nTo view the pointclouds in RViz2:")
print("$ ros2 run rviz2 rviz2")
print("Set 'Fixed Frame' to 'world' and add PointCloud2 displays for the topics")
