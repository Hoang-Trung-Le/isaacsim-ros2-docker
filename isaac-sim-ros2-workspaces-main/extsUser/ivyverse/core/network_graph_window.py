...
class RunnableNetworkDelegateClassic(GraphNodeDelegate):
    ...
    def connection(self, model, source: GraphConnectionDescription, target: GraphConnectionDescription):
        ...
        agent = source.node
        if isinstance(agent.outputs, BaseMessage) and agent.outputs.content:
            tooltip = f"{agent.outputs.content}"
        elif isinstance(agent.outputs, BaseMessage):
            tooltip = agent.outputs.json(indent=2)
        else:
            tooltip = f"{agent.outputs}"
