...
class AsyncUSDClassAppender(BaseModel, RunnableLambda):
    ...
    async def _append_relevant_classes(self, input):
        ...
        found_classes = await self._fetch_relevant_classes_from_network(
            prompt, default_node, chat_model_name
        )
        if found_classes:
    ...
    def _prepare_prompt(self, classes: str) -> str:
        ...
        return USD_CLASS_QUERY_PROMPT.replace("{question}", self.question).replace(
            "{classes}", classes
        )
    ...
    async def _fetch_relevant_classes_from_network(
        ...
        with RunnableNetwork(
            chat_model_name=chat_model_name, default_node=default_node
        ) as network:
    ...
    def _create_message_from_classes(self, classes: list) -> BaseMessage:
        ...
        return convert_to_message(
            (self.type, CLASSES_MESSAGE.replace("{classes}", found_classes_text))
        )
...
class CodeAtlasErrorMessage(BaseRetrieverMessage):
    ...
    def _lookup_module(self, module_name: str, attribute: str) -> Optional[str]:
        return _get_code_atlas_tool().cache.lookup_module(
            module_name, classes=True, methods=False, docs=False
        )
