...
def _cull_messages(messages, max_tokens, tokenizer, remove_from_start=True):
    ...
    system_indices = [i for i in range(len(messages)) if isinstance(messages[i], SystemMessage)]
    other_indices = [i for i in range(len(messages)) if i != latest_message_index and i not in system_indices]
