...
class LayoutDelegate(AgentDelegate):
    ...
    def build_request_buttons(self, network, input_model):
        ...
        with ui.HStack():
            ...
            with ui.VStack(width=0, content_clipping=True):
                ui.Spacer()
                ui.Button(name="upload", height=32, width=32, clicked_fn=_on_input_image_button_clicked)
                ui.Spacer()
            ui.Spacer(width=2)
            with ui.ZStack():
        ...
        with ui.VStack():
            ...
            if agent.name != "UserAgent":
                with ui.HStack(height=40):
                    ...
                    ui.Spacer(width=20)
                    ui.Button(
                        name="Thumb-up", width=30  # , clicked_fn=lambda n=network, a=agent: send_rated_event(n, a, 1)
                    )
                    ui.Button(
                        name="Thumb-down", width=30  # , clicked_fn=lambda n=network, a=agent: send_rated_event(n, a, 0)
                    )
                    # ui.Button(
                    #     name="show-systems",
                    #     width=30,
